...
def main(log_csv_name):
    ...
    old_to_new_names = df_moves.fname.apply(_deconstruct_git_move).values
    # They have to be unpacked in two separate lists
    olds, news = list(zip(*list(old_to_new_names)))
    ...
    df.to_csv(
        log_csv_name.replace(".csv", "_repaired.csv"),
        index=False,
